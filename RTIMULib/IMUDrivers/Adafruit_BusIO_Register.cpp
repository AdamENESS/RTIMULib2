...
Adafruit_BusIO_Register::Adafruit_BusIO_Register(
	...
	Adafruit_BusIO_SPIRegType type,
		uint16_t slave_addr, uint16_t reg_addr,uint8_t width,
	uint8_t byteorder, uint8_t address_width) {
	_i2cdevice = i2cdevice;
	_spiregtype = type;
	_addrwidth = address_width;
	_address = reg_addr;
	_slaveAddress = slave_addr;
...
bool Adafruit_BusIO_Register::write(uint8_t *buffer, uint8_t len) {
	...
	if (_i2cdevice)
	{
		return _i2cdevice->HALWrite(_slaveAddress, _address, len, buffer,"Adafruit_BusIO_Register::Write" );
	}
	// if (_i2cdevice) {
...
bool Adafruit_BusIO_Register::read(uint8_t *buffer, uint8_t len) {
	...
	if (_i2cdevice)
	{
		return _i2cdevice->HALRead(_slaveAddress, _address, len, buffer,"Adafruit_BusIO_Register::Read" );
	}
	// if (_i2cdevice) {
